...
func (l *LogApp) RequestNum(uid uint64) (vo.RequestNum, int) {
	...
	if err != nil {
		...
		log.Error("get Log RequestNum requestTotal err", err)
	}
	// 有效的请求
	...
	if err != nil {
		validTotal = 0
		ret = interfaces.ErrorRequestNum
		log.Error("get RequestNum validTotal", err)
	}
...
func (l *LogApp) QPSPeak(uid uint64) (vo.QPSPeak, int) {
	...
	if err != nil {
		log.Error("get Log qps peak err", err)
...
func (l *LogApp) SevenDaysRequestNumList(uid uint64, startTime, endTime int64) (res vo.LogRequestSevenDays, ret int) {
	...
	if err != nil {
		log.Error("get SevenDaysQPSGroupBYDay totalRequestNumList fail", err)
	...
	if err != nil {
		log.Error("get SevenDaysQPSGroupBYDay validRequestNumList fail", err)
		ret = interfaces.ErrorSevenRequestNum
		return
	}
